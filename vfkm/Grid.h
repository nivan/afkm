...
struct Segment {
    ...
    float time[2];
    int index;
    //std::vector<Vector> attributeVectors;//this is indexed by dimension (or attribute) index
    //each entry is a vector containing the values of
    //the attributes in the segment ends and 0 otherwise
    ...
    //
    inline void getFieldValueOnEndPoints(const std::vector<Vector> &attributeField,
                                         Vector& v0,
                                         Vector& v1) const
    {
        //
        v0.setValues(0.0);
        v1.setValues(0.0);
	int numAttFields = attributeField.size();
        for(int d = 0 ; d < numAttFields ; ++d){
            const Vector& fd = attributeField.at(d);
            //
            v0[d] = fd[endpoint[0].face.indexV1] * endpoint[0].barycentric_coords[0]+
                    fd[endpoint[0].face.indexV2] * endpoint[0].barycentric_coords[1]+
                    fd[endpoint[0].face.indexV3] * endpoint[0].barycentric_coords[2];

            v1[d] = fd[endpoint[1].face.indexV1] * endpoint[1].barycentric_coords[0]+
                    fd[endpoint[1].face.indexV2] * endpoint[1].barycentric_coords[1]+
                    fd[endpoint[1].face.indexV3] * endpoint[1].barycentric_coords[2];
        }
    }
};
...
struct AFKMCurveDescription
{
    ...
    //
    std::vector<Vector> verticesAttributes; // attributes for each segment endpoint in the trajectory

    //this stores the multiplication of the trajectory
    //matrix by each of the target fields
...
public:
    ...
    inline bool contains(double x, double y) const {
        bool conditionX = m_x <= x && x <= (m_x + m_w);
        bool conditionY = m_y <= y && y <= (m_y + m_h);

        return conditionX && conditionY;
    }
    inline bool contains(Vector2D p) const {
        bool conditionX = m_x <= p.X() && p.X() <= (m_x + m_w);
        bool conditionY = m_y <= p.Y() && p.Y() <= (m_y + m_h);
    ...
    void multiplyByLaplacianResult(Vector&, const Vector&) const; //multiply second field by the laplacian and put the return on the first
    ...
    void clipLine(AttributeTrajectory&) const;


    CurveDescription curve_description(const PolygonalPath &curve) const;
    AFKMCurveDescription curve_description(const AttributeTrajectory &curve) const;
