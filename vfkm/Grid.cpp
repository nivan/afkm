...
void Grid::multiplyByLaplacianResult(Vector& result, const Vector& field) const{
    int numberOfVertices = m_resolutionX * m_resolutionY;
    if(numberOfVertices != result.getDimension() ||
            numberOfVertices != field.getDimension()){
        cout << "Error while multiplying grid by vector. Incompatible dimensions." << endl;
        exit(1);
    }
    ...
    //numberOfVertices in the grid == resolution * resolution
    for(int i = 0 ; i < numberOfVertices ; ++i){
        ...
        //newVector.add(vectorField.at(i) * (-degree));
        result[i] = accum1 - degree * field[i];
    }
}
...
AFKMCurveDescription Grid::curve_description(const AttributeTrajectory &path) const
{
    AFKMCurveDescription result;
#if 0
    assert(false); //TODO
#else
    result.length = 0;
    ...
    int noAttributes = path.getNumAttributes();
    result.rightHandSides.clear();
    for(int d = 0 ; d < noAttributes ; ++d){
        Vector rhs(2*(numberOfPoints-1));
    ...
    for(int i = 0 ; i < numberOfPoints - 1 ; ++i){
        ...
        //
        Vector endPoint0AttVec = path.getAttributeVector(i);
        Vector endPoint1AttVec = path.getAttributeVector(i+1);
        for(int d = 0 ; d < noAttributes ; ++d){
            Vector &rhs = result.rightHandSides.at(d);
            rhs[2*i]   = endPoint0AttVec[d];
            rhs[2*i+1] = endPoint1AttVec[d];
        }
...
//////////////////////////////////////
void Grid::clipLine(AttributeTrajectory &path1) const
{
    size_t currentVertexIndex = 0;
    ...
    while(currentVertexIndex < path1.numberOfPoints() - 1){
...
#ifdef DEBUG
        cout << "CurrentVertexIndex " << currentVertexIndex << " pathNumPoints " << path1.numberOfPoints() << endl;
        cout << "   Grid Coords " << e1.grid_point.toString() << " " << e2.grid_point.toString() << endl;
        cout << "   Horizontal Clip " << horiz.size() << endl;
#endif
        ...
        for (size_t i=1; i<inters.size()-1; ++i) {
            ...
            const Vector v1 = path1.getAttributeVector(currentVertexIndex);
            const Vector v2 = path1.getAttributeVector(currentVertexIndex+1);
            Vector v3(v1.getDimension());
            Vector::interpolate(v1,v2,inters[i].u,v3);
            //
            pair<Vector2D,float> newPoint = make_pair(toWorld(inters[i].grid_point),
                                                      inters[i].u * tto + (1 - inters[i].u) * tfrom);
            path1.addPoint(++currentVertexIndex,newPoint,v3);
        }
        currentVertexIndex++;
    }
}
